...
public enum UMaterials implements Versionable {
    /*
    ...
    */
    ...
    ;
    private static final HashMap<String, UMaterials> CACHE = new HashMap<>();
    ...
    private byte data;
    UMaterials(String ...names) {
        this(0, names);
    }
    UMaterials(String name, int data) {
        names[0] = name;
        versionName = name;
        this.data = (byte) data;
    }
    UMaterials(String name, int spoofedData, Color color) {
        names[0] = name;
        versionName = name;
        this.data = (byte) spoofedData;
        attributes = "color=" + color.getRed() + ":" + color.getGreen() + ":" + color.getBlue();
    }
    UMaterials(PotionBase base, String type, boolean extended, boolean upgraded, String ...names) {
        this.names = names;
        attributes = "upotion=" + base.name() + ":" + type + ":" + extended + ":" + upgraded;
    }
    UMaterials(String name, Enchantment enchant, int level) {
        names[0] = name;
        versionName = name;
        data = 0;
        attributes = "enchant=" + enchant.getName() + ":" + level;
    }
    UMaterials(int data, String ...names) {
        this.names = names;
        this.data = (byte) data;
    }
    ...
    @Deprecated
    public static UMaterials match(String name, byte data) {
        name = name.toUpperCase();
        if(CACHE.containsKey(name + data)) return CACHE.get(name + data);
        for(UMaterials u : values()) {
            if(u.getData() == data) {
    ...
    public static UMaterials matchSpawnEgg(ItemStack egg) {
        if(egg != null) {
            if(EIGHT) {
    ...
    public static UMaterials matchEnchantedBook(ItemStack book) {
    ...
    public static UMaterials matchPotion(ItemStack potion) {
        if(potion != null && potion.getItemMeta() instanceof PotionMeta) {
            ...
            if(ce.size() == 0) {
                ...
                if(type != null) {
                    ...
                    return UMaterials.POTION;
                }
            }
        }
        return null;
    }
    public static UMaterials match(ItemStack item) {
        if(item != null && !item.getType().equals(Material.AIR)) {
            ...
            if(EIGHT || NINE || TEN || ELEVEN || TWELVE) {
                ...
                final UMaterials u = match(un, d), potion = u == null ? matchPotion(item) : null;
    ...
    public static UMaterials getItem(Block block) {
        ...
        if(!EIGHT && !NINE && !TEN && !ELEVEN && !TWELVE || m.contains("TERRACOTTA") || m.startsWith("TORCH") || m.startsWith("REDSTONE_TORCH") || m.contains("STAIRS") || m.equals("FLINT_AND_STEEL") || m.equals("SOIL") || m.equals("LADDER") || m.equals("BONE_BLOCK") || m.equals("OBSERVER") || m.contains("FENCE_GATE") || m.contains("TRAPDOOR") || m.contains("CHEST")
            ...
            if(d == 8) {
                t = UMaterials.ACACIA_WOOD;
            } else if(d == 12) {
                t = log2 ? UMaterials.ACACIA_WOOD : UMaterials.OAK_WOOD;
            } else if(d == 13) {
                t = log2 ? UMaterials.DARK_OAK_WOOD : UMaterials.SPRUCE_WOOD;
            } else if(d == 14) {
                t = UMaterials.BIRCH_WOOD;
            } else if(d == 15) {
                t = UMaterials.JUNGLE_WOOD;
            }
            ...
            if(TWELVE) {
                ...
                return UMaterials.WHITE_BED;
            }
        } else if(m.contains("CROP")) {
            return UMaterials.AIR;
        } else if(m.contains("PISTON_")) {
            return m.contains("_STICKY_") ? UMaterials.STICKY_PISTON : UMaterials.PISTON;
...
class UPotion implements Versionable {
    ...
    @SuppressWarnings("deprecation")
	public UPotion(UMaterials.PotionBase base, String type, boolean extended, boolean upgraded) {
        this.base = base;
    ...
    public UMaterials.PotionBase getBase() { return base; }
