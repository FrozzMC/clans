...
public class BankCommand extends AdminCommands
{
	private String name;
	public BankCommand(EpicSetClans plugin,EpicClanManager clanManager,ClanDuelManager duelManager) 
	{
	...
	public void onCommandByPlayer(Player sender, String[] sub) 
	{
		if(sub.length>3&&sub[1].equalsIgnoreCase("give"))
		{
            ...
            if(clanManager.getClanData(clanName)!=null)
            {
    			...
    			{
    				sender.sendMessage(CommonUtils.chat("&bClanAdmin &7: &fPlease enter correct amount."));
    				return;
    			}
    			amount=Math.abs(amount);
            	clan.setClanBalance(clan.getClanBalance()+amount);
				sender.sendMessage(CommonUtils.chat("&bClanAdmin &7: &fGiven "+amount+" $ to "+clan.getClanName()+"&f's bank."));
				return;
            }
            else
            {

				sender.sendMessage(CommonUtils.chat("&bClanAdmin &7: &fCannot find a clan by that name."));
				return;
            }
		...
		if(sub.length>3&&sub[1].equalsIgnoreCase("take"))
		{
            ...
            if(clanManager.getClanData(clanName)!=null)
            {
    			...
    			amount=Math.abs(amount);
    			long total = clan.getClanBalance()-amount;
    			if(total<0)
    				total=0;
            	clan.setClanBalance(total);
				sender.sendMessage(CommonUtils.chat("&bClanAdmin &7: &fTaken "+amount+" $ from "+clan.getClanName()+"&f's bank."));
				return;
            }
            else
            {
		...
		sender.sendMessage(CommonUtils.chat("&7- &f/clanadmin bank give [amount] [clanname/membername]  &bgive money to bank"));
		sender.sendMessage(CommonUtils.chat("&7- &f/clanadmin bank take [amount] [clanname/membername] &btake money from bank"));
	}
